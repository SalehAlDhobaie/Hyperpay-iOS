...
/**
 A convenience method to check if Touch ID or passcode are set on user device.
 @return NO if neither Touch ID nor passcode are set, otherwise YES.
 */
 ...
 */
- (void)requestCheckoutInfoWithCheckoutID:(NSString *)checkoutID completionHanler:(void (^)(OPPCheckoutInfo * _Nullable checkoutInfo, NSError * _Nullable error))completionHandler;
